...
class Submit(Base):
    ...
    acm = Column(Boolean)
    subsection_id = Column(Integer, ForeignKey('subsections.id'))
    isomorphism_checked = Column(Boolean)
...
class News_section(Base):
    __tablename__ = 'news_section'

    id = Column(Integer, Sequence('news_section_id_seq'), primary_key=True)
    context = Column(String)
    date = Column(DateTime(timezone=True))
    section_id = Column(Integer, ForeignKey('sections.id'))

class News_subsection(Base):
    __tablename__ = 'news_subsection'

    id = Column(Integer, Sequence('news_subsection_id_seq'), primary_key=True)
    context = Column(String)
    date = Column(DateTime(timezone=True))
    subsection_id = Column(Integer, ForeignKey('subsections.id'))
...
class Isomorphism(Base):
    ...
    id = Column(Integer, Sequence('isomorphism_id_seq'), primary_key=True)
    percentage = Column(Integer)
    old_submit_id = Column(Integer, ForeignKey('submits.id'))
    new_submit_id = Column(Integer, ForeignKey('submits.id'))
...
engine = create_engine('postgresql+psycopg2://{}:{}@{}/{}'.format(NEMESIS_USER, NEMESIS_PASSWD, NEMESIS_HOST, NEMESIS_DB), encoding="utf8")
...
def init_db():
    ...
    News_section.metadata.create_all(engine)
    News_subsection.metadata.create_all(engine)
    Isomorphism.metadata.create_all(engine)
